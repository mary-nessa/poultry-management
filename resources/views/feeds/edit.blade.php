...
<div class="container">
    ...
    <form action="{{ route('feeds.update', $feed->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="type" id="type" class="mt-1 block w-full border-gray-300 rounded-md" value="{{ old('type', $feed->type) }}" required>
        </div>

        <div class="mb-4">
            <label for="quantity_kg" class="block text-sm font-medium text-gray-700">Quantity (kg)</label>
            <input type="number" name="quantity_kg" id="quantity_kg" class="mt-1 block w-full border-gray-300 rounded-md" value="{{ old('quantity_kg', $feed->quantity_kg) }}" required>
        </div>

        <div class="mb-4">
            <label for="unit_cost" class="block text-sm font-medium text-gray-700">Unit Cost</label>
            <input type="number" name="unit_cost" id="unit_cost" class="mt-1 block w-full border-gray-300 rounded-md" value="{{ old('unit_cost', $feed->unit_cost) }}" required>
        </div>

        <div class="mb-4">
            <label for="purchase_date" class="block text-sm font-medium text-gray-700">Purchase Date</label>
            <input type="date" name="purchase_date" id="purchase_date" class="mt-1 block w-full border-gray-300 rounded-md" value="{{ old('purchase_date', $feed->purchase_date) }}" required>
        </div>

        <div class="mb-4">
            <label for="expiry_date" class="block text-sm font-medium text-gray-700">Expiry Date</label>
            <input type="date" name="expiry_date" id="expiry_date" class="mt-1 block w-full border-gray-300 rounded-md" value="{{ old('expiry_date', $feed->expiry_date) }}">
        </div>
